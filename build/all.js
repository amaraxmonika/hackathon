...
var socket = io('http://beardownforwhat.com');
...
socket.on('connect', function(){
    ...
    delivery.on('receive.success', function(file){
        console.log('file download done' + file.dataURL());
        //uriContent = "data:application/octet-stream," + encodeURIComponent(file);
        ...
        download("lefilenname.png", file.dataURL());

        //temp.

    });
});
socket.on('windowRedirect', function (data) {
    console.log('Redirect here: ' + data );
    //socket.emit('newData', { my : "data is awesome" });
    if (data.key != null)
...
var EMOTICONS = ['happy', 'sad', 'heart', 'mad', 'star'];
//var EMOTICONS = ['happy', 'sad', 'heart', 'mad', 'star', 'oh', 'more', 'hi', 'thisasdf'];
...
// Setting up server and socket connections
// and starting
...
// Generate alphabet to be used for encoding
...
function registerButtonListener2() {
    
    var myButton = document.getElementById('myButton2');
    myButton.addEventListener('click', function () {
        //alert('this works');
      //sonicSocket.send("1327");
      sonicSocket.send('2');
      //var temp = parseInt(Math.random() * 1000);
      //sonicSocket.send(temp);
      //socket.emit('redirect', { key: '172' });
      //socket.emit('codeEvent', { key: temp });
...
function registerButtonListener() {
    ...
    myButton.addEventListener('click', function () {
      ...
      //var temp = parseInt(Math.random() * 1000);
      //sonicSocket.send(temp);
      socket.emit('codeEvent', { key: '1' });
      //socket.emit('codeEvent', { key: '172' });
      //socket.emit('codeEvent', { key: temp });
...
function onIncomingEmoticon(message) {
  ...
  socket.emit('codeEvent', { key : message });
  if (message == 2) 
      socket.emit('redirect', { key : message });

  //socket.emit('newData', { someData : message });
  //socket.emit('Data', { someData : message });
  //socket.emit('codeEvent', { key: '172' });
